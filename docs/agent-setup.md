...
termyard tracks AI coding agents running inside your tmux sessions. Each agent needs a hook configured so it can notify termyard of status changes (active, waiting for input, completed, error).

## Session labels

Termyard keeps two session fields:
...
```bash
termyard agent-setup --dry-run
```

## Resilience

By default, `termyard agent-setup` appends `|| true` to all hook commands so that failures (termyard binary missing, server down, etc.) never block the agent. This means hooks are fire-and-forget — if termyard isn't reachable, the agent continues working normally.

Additionally, `termyard notify` uses 1-second timeouts for both unix socket and HTTP connections, so even without `|| true` the worst-case delay is ~2 seconds.

To disable this and let hook failures propagate to the agent:

```bash
termyard agent-setup --block
```

**When to use `--block`:** If you want the agent to be aware that termyard notifications are failing (e.g., for debugging). In normal use, the default non-blocking behavior is recommended.
...
**Auto-configured by `termyard agent-setup`.**

termyard adds hooks to `~/.claude/settings.json` that fire on tool use, notifications (permission prompts, input dialogs), and task completion. The Stop hook reads Claude's transcript file to capture the last assistant message, so Claude sessions show a meaningful last message instead of only "Task complete".
    ...
    "UserPromptSubmit": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "termyard notify -t claude -s active -m 'Thinking' --stdin || true"
          }
        ]
      }
    ],
    ...
    "Notification": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "termyard notify -t claude -s waiting -m 'Permission needed' || true"
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "termyard notify -t claude -s waiting -m 'Needs input' || true"
          }
        ]
      }
    ],
    "Stop": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "termyard notify -t claude -s completed --stdin || true"
          }
        ]
      }
    ]
...
```toml
model = "o4-mini"
notify = ["bash", "-c", "termyard notify -t codex --event-data \"$1\" || true", "--"] # termyard-agent-hook
...
Codex passes a JSON blob as an additional argument when the agent completes a turn and needs user input. The `--event-data` flag tells termyard to parse this JSON natively. Fields extracted:
...
The `agent-turn-complete` event maps to termyard's `waiting` status, which triggers an alert in the UI and a push notification.
...
termyard writes a JavaScript plugin to `~/.config/opencode/plugins/termyard.js` that hooks into OpenCode's event system. The plugin registers handlers for `permission.ask`, `permission.asked`, `permission.replied`, `command.execute.before`, `tool.execute.before/after`, `session.idle`, and `session.error`.

**Manual setup:** Create `~/.config/opencode/plugins/termyard.js` with a Bun-compatible plugin module that calls `termyard notify` for each event type:

| OpenCode Event           | termyard Status | Message             |
| ------------------------ | ------------ | ------------------- |
...
Run `termyard agent-setup` to generate the plugin file automatically.

### Pi

**Auto-configured by `termyard agent-setup`.**

termyard writes a TypeScript extension to `~/.pi/agent/extensions/termyard.ts`.

| Pi Event                   | termyard Status | Message                               |
| -------------------------- | ------------ | ------------------------------------- |
...
Under the hood, all agent hooks call `termyard notify`. You can also use it directly:
...
echo '{"hook_event_name":"Stop","last_assistant_message":"Done"}' | termyard notify -t codex --stdin
```
...
| `--server`  |       | termyard server URL (default: `http://localhost:7654`) |
| `--socket`  |       | Unix socket path (auto-detected)                    |

**Communication:** `termyard notify` tries the Unix socket first (zero-config when termyard server is running locally), then falls back to HTTP. Both use 1-second timeouts to minimize impact on agent performance.
...
To bridge this gap, termyard includes an **inactivity promoter**: if a non-Claude tool sends "active" events but then goes quiet for 30 seconds, termyard automatically generates a synthetic "waiting" event with the message "May need attention". This surfaces the alert in the UI and triggers push notifications, just like a native waiting event would.
